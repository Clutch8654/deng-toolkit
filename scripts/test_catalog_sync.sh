...
#
# Creates isolated test environment, runs all tests, cleans up.
...
setup_test_env() {
    rm -rf "$TEST_DIR"
    mkdir -p "$TEST_DIR/.deng-toolkit/catalog"
...
# ============================================================
# AC1: Rejects paths outside ~/.deng-toolkit/catalog
# ============================================================
test_ac1_rejects_bad_paths() {
    setup_test_env

    export DENG_CATALOG_DIR="/tmp/evil"
    if "$SCRIPT_UNDER_TEST" --status 2>&1 | grep -q "Error"; then
        log_pass "AC1: Rejects paths outside ~/.deng-toolkit/catalog"
    else
        log_fail "AC1: Rejects paths outside ~/.deng-toolkit/catalog"
    fi
...
# ============================================================
# AC2: Accepts default path
# ============================================================
test_ac2_accepts_default_path() {
    setup_test_env

    # Initialize first so we have a valid repo
    "$SCRIPT_UNDER_TEST" --init >/dev/null 2>&1

    # Now test status works
    if "$SCRIPT_UNDER_TEST" --status >/dev/null 2>&1; then
        log_pass "AC2: Accepts default path ~/.deng-toolkit/catalog"
    else
        log_fail "AC2: Accepts default path ~/.deng-toolkit/catalog"
    fi
...
test_ac3_init_creates_git() {
    ...
    if [[ -d "$TEST_DIR/.deng-toolkit/catalog/.git" ]]; then
...
test_ac4_init_creates_gitignore() {
    ...
    GITIGNORE="$TEST_DIR/.deng-toolkit/catalog/.gitignore"
    if [[ -f "$GITIGNORE" ]] && \
...
test_ac5_init_idempotent() {
    ...
    if [[ $EXIT_CODE -eq 0 ]] && echo "$OUTPUT" | grep -q "Already a git repo"; then
...
test_ac6_status_shows_git_status() {
    ...
    echo "test" > "$TEST_DIR/.deng-toolkit/catalog/test.yaml"
...
test_ac9_push_commits_changes() {
    ...
    "$SCRIPT_UNDER_TEST" --init >/dev/null 2>&1
    echo "new content" > "$TEST_DIR/.deng-toolkit/catalog/tables.yaml"

    "$SCRIPT_UNDER_TEST" --push >/dev/null 2>&1

    # Check git log for new commit
    cd "$TEST_DIR/.deng-toolkit/catalog"
...
test_ac10_push_commit_message_date() {
    ...
    "$SCRIPT_UNDER_TEST" --init >/dev/null 2>&1
    echo "content" > "$TEST_DIR/.deng-toolkit/catalog/columns.yaml"
...
test_ac11_pull_no_remote() {
    ...
    # Should contain message about failed/no remote, but not crash
    if echo "$OUTPUT" | grep -qi "fail\|no remote"; then
...
# ============================================================
# AC13: Missing catalog directory fails
# ============================================================
test_ac13_missing_catalog_fails() {
    setup_test_env
    rm -rf "$TEST_DIR/.deng-toolkit/catalog"

    OUTPUT=$("$SCRIPT_UNDER_TEST" --status 2>&1) || true

    if echo "$OUTPUT" | grep -qi "not found\|error"; then
        log_pass "AC13: Missing catalog directory fails with error"
    else
        log_fail "AC13: Missing catalog directory fails with error"
    fi
