---
name: deng-catalog-refresh
description: Use when database metadata catalog is missing or stale (>7 days old). Heavy operation - runs DB scans. Only run when explicitly requested or catalog is missing.
---
...
```
/deng-catalog-refresh [--target NAME]
```
...
uv run --with pymssql --with polars --with python-dotenv \
    python ~/.deng-toolkit/scripts/catalog_refresh.py --target oms
```

For all targets:
```bash
uv run --with pymssql --with polars --with python-dotenv \
    python ~/.deng-toolkit/scripts/catalog_refresh.py
```
...
```bash
uv run --with polars \
    python ~/.deng-toolkit/scripts/catalog_snapshot.py
```
