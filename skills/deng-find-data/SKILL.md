---
name: deng-find-data
description: Use when searching for database tables/columns by keyword. Fast catalog lookup - no DB access required. Use this BEFORE writing any SQL queries.
---
...
```bash
python ~/.deng-toolkit/scripts/catalog_query.py --status
```
...
```bash
uv run --with polars python ~/.deng-toolkit/scripts/catalog_query.py "<keywords>" --top 20
```
...
1. **For table details:** `python ~/.deng-toolkit/scripts/catalog_query.py --table Database.TableName`
2. **For join paths:** `python ~/.deng-toolkit/scripts/catalog_query.py --joins TableName`
...
```bash
uv run --with polars python ~/.deng-toolkit/scripts/catalog_query.py "order cancel" --format json
```
...
## Integration with DS Workflow

### In Feature Engineering

Before creating a feature, search for relevant tables:

```
/deng-find-data payment history
```

Then validate time windows:
- Check for timestamp columns in results
- Verify data availability before feature time origin

### In Cohort Construction

Find entity and event tables:

```
/deng-find-data company account customer
/deng-find-data order status activation
```

### In Data Exploration

Before writing exploration SQL, find candidate tables:

```
/deng-find-data quote order conversion
```

## Project Catalog

If working in a DS project with a catalog snapshot:

```bash
# Check project catalog
ls artifacts/catalog/

# Read project summary
cat artifacts/catalog/DATA_CATALOG_SUMMARY.md
```

Project snapshots provide reproducibility - the same catalog version used throughout the analysis.
