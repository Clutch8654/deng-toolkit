...
```bash
uv run --with polars python ${CLAUDE_PLUGIN_ROOT}/scripts/build_ontology.py
```
...
If the MCP server is running, use `query_ontology` for semantic queries.

## Troubleshooting

**"Config not found" error:**
- Ensure `~/.ds_catalog/ontology_config.toml` exists
- Copy from template: `cp ${CLAUDE_PLUGIN_ROOT}/templates/ontology_config.toml.example ~/.ds_catalog/ontology_config.toml`
