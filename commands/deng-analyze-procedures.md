...
uv run --with pymssql --with polars --with python-dotenv \
    python ${CLAUDE_PLUGIN_ROOT}/scripts/analyze_procedures.py --target oms

# Analyze all targets
uv run --with pymssql --with polars --with python-dotenv \
    python ${CLAUDE_PLUGIN_ROOT}/scripts/analyze_procedures.py

# Deep analysis on top 50 procedures only
uv run --with pymssql --with polars --with python-dotenv \
    python ${CLAUDE_PLUGIN_ROOT}/scripts/analyze_procedures.py --deep-analysis 50

# Skip SSRS log analysis
uv run --with pymssql --with polars --with python-dotenv \
    python ${CLAUDE_PLUGIN_ROOT}/scripts/analyze_procedures.py --skip-ssrs
```
